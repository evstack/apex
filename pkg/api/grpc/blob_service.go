...
func (s *BlobServiceServer) Get(ctx context.Context, req *pb.GetBlobRequest) (*pb.Blob, error) {
	ns, err := bytesToNamespace(req.Namespace)
	if err != nil {
		return nil, status.Errorf(codes.InvalidArgument, "invalid namespace: %v", err)
	}

	blobs, err := s.svc.Store().GetBlobs(ctx, ns, req.Height, req.Height)
	if err != nil {
	...
	for i := range blobs {
		if bytes.Equal(blobs[i].Commitment, req.Commitment) {
			return blobToProto(&blobs[i]), nil
		}
	}
...
func (s *BlobServiceServer) Subscribe(req *pb.SubscribeBlobsRequest, stream grpc.ServerStreamingServer[pb.BlobEvent]) error {
	nsList := make([]types.Namespace, len(req.Namespaces))
	for i, nsBytes := range req.Namespaces {
		ns, err := bytesToNamespace(nsBytes)
		if err != nil {
			return status.Errorf(codes.InvalidArgument, "invalid namespace %d: %v", i, err)
		}
		nsList[i] = ns
	}

	sub := s.svc.Notifier().Subscribe(nsList)
	...
	for {
		...
		case ev, ok := <-sub.Events():
			...
			if err := stream.Send(&pb.BlobEvent{
				Height: ev.Height,
				Blobs:  pbBlobs,
			}); err != nil {
				return err
